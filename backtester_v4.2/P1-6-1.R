...
lookbackWRL <- seq(from=23, to=27, by=2)
lookbackS <- seq(from=7, to=9,by=1)
#lookbackS <- 9
lookbackL <- seq(from=13, to=17, by=2)
#lookbackL <- 10
threshold <- seq(from=28,to=32,by=2)
#threshold<-30
      ...
      for (lbr in lookbackWRL) {
        for (th in threshold) {
          ...
          params <- list(lookbackS=lbs,lookbackL=lbl,lookbackWRL=lbr,
                         threshold=th,series=8,posSizes=rep(1,10)) 
