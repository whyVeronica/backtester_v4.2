...
getOrders <- function(store, newRowList, currentPos, params) {
  ...
  if (store$iter > lookback) {
    ...
    for (i in 1:length(params$series)) {
      ...
      maAvgLogRet[i] <- mean(abs(diff(log(store$cl[startIndex:store$iter,i]))))
      #print(maAvgLogRet[i])
      ...
      spread <- sapply(1:length(newRowList),function(i)
        maAvgLogRet[i] * (newRowList[[i]]$High - newRowList[[i]]$Low))
      
      MAPriceBase <- last(SMA(store$cl[startIndex:store$iter,i],n=params$lookback))
      ...
      if (params$nHold == 3){
        if (last(macd) > last(signal) && last(macd,n=2)[-2] > last(signal,n=2)[-2]
          ...
          marketOrders[params$series[i]] <- positionSizes[i] # long
        limitOrders2[i]  <- positionSizes[i] # BUY LIMIT ORDERS
        limitPrices2[i]  <- sapply(1:length(newRowList),function(i) 
          MAPriceBase + spread[i]/2)
