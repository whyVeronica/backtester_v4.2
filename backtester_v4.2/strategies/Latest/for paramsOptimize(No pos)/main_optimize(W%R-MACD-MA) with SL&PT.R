...
source('strategies/W%R-MACD-MA4-stoploss-profitTarget.R') 
...
lossLimits <- seq(from=100, to=10000, by=500)
profitTarget <- seq(from=100, to=10000, by=500)
                ...
                for (ll in lossLimits){
                   for (pt in profitTarget){
                      params <- list(lookbackS=lbs,lookbackL=lbl,lookbackR=lbr,
                                     ...
                                     series=c(1:10),posSizes=rep(1,10)) 
