...
getOrders <- function(store, newRowList, currentPos, params) {
  ...
  if (store$iter >= max(params$lookbackWRL,params$lookbackL,params$lookbackS,lookback)) {
    ...
    for (i in 1:length(params$series)) {
      ...
      #print(CPnL)
      if(CPnL[params$series[i]] < -params$lossLimits || CPnL[params$series[i]] > params$profitTarget) {
      ...
      posPnL[params$series[i]] <- ifelse(PnLCurrent[store$iter,i] > 0,PnLCurrent[store$iter,i],1)
      ...
      highma <- SMA(store$hi[startIndexLimit:store$iter,i],n=params$lookbackWRL)
      lowma <- SMA(store$lo[startIndexLimit:store$iter,i],n=params$llookbackWRL)
      MALogRet[params$series[i]] <- last(SMA(abs(diff(log(store$cl[startIndexLimit:store$iter,i]))),n=params$lookbackWRL))
      #print(maxLogRet)
      spread <- allzero
      
      spread[params$series[i]] <- MALogRet[params$series[i]] * (last(highma) - last(lowma))
