...
initialLongPrice1 <- initialLongPrice2 <- rep(0,10) 
initialShortPrice1 <- initialShortPrice2 <- rep(0,10)
longCount1 <- longCount2 <- shortCount1 <- shortCount2 <- rep(0,10)
...
getOrders <- function(store, newRowList, currentPos, params) {
  ...
  actionIndex1 <- actionIndex2 <- matrix(0,nrow=maxRows,ncol=10)
    ...
    for (i in 1:length(params$series)) {
      ...
      if (R1 < -50 - params$threshold1){
        ...
        if (last(MAclS1) > last(MAclL1) && last(MAclS1,n=2)[-2] < last(MAclL1,n=2)[-2] 
          ...
          longIndex1 <- store$iter
          actionIndex1[store$iter,params$series[i]] <- 1
          #initialLongPrice[params$series[i]] <<- ifelse(0,cl,initialLongPrice[params$series[i]]+cl)
      ...
      if (R2 < -50 - params$threshold2){
        ...
        if (last(MAclS2) > last(MAclL2) && last(MAclS2,n=2)[-2] < last(MAclL2,n=2)[-2] 
          ...
          longIndex2 <- store$iter
          actionIndex2[store$iter,params$series[i]] <- 1
          #initialLongPrice[params$series[i]] <<- ifelse(0,cl,initialLongPrice[params$series[i]]+cl)
