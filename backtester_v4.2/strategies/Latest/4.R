    ...
    for (i in 1:length(params$series)) {
      ...
      if (R1 < -50 - params$threshold1){
        ...
        if (last(MAclS1) > last(MAclL1) && last(MAclS1,n=2)[-2] < last(MAclL1,n=2)[-2] 
          ...
          currentCashFlow12[store$iter,params$series[i]] <- -cl*positionSizes[i]
      ...
      cashFlow1[store$iter,params$series[i]] <<- cashFlow1[(store$iter-1),params$series[i]] 
      + currentCashFlow11[store$iter,params$series[i]] + currentCashFlow12[store$iter,params$series[i]]
      cat("cashFlow1:",cashFlow1[store$iter,params$series[i]],"\n")
      #PnLOld[params$series[i]] <- PnL[params$series[i]] #Yesterday's profit/loss
      PnLNew1[params$series[i]] <- currentWorth[params$series[i]] +  cashFlow1[store$iter,params$series[i]]
...
cat("cashFlow2:",cashFlow2[store$iter,params$series[i]],"\n")
        #PnLOld[params$series[i]] <- PnL[params$series[i]] #Yesterday's profit/loss
        PnLNew2[params$series[i]] <- currentWorth[params$series[i]] +  cashFlow2[params$series[i]]
