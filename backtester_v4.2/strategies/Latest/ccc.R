  ...
  if (store$iter > max(params$lookback_macd,lookbackm)) {
      ...
      
    {
      if(i==4){
        ...
        startIndex_macd <-  store$iter - params$lookback_macd[i]
        startIndex1_macd <-  store$iter - params$lookback1_macd[i]
        startIndex2_macd <-  store$iter - params$lookback2_macd[i]
        ...
        avgAbsDiffs <- mean(absOpenDiffs)
        largestAvgAbsDiff <- max(avgAbsDiffs)
        posSizes[params$series[i]] <- round(largestAvgAbsDiff/avgAbsDiffs[i])
        op <- newRowList[[params$series[i]]]$Open         
        cl <- newRowList[[params$series[i]]]$Close
        rsi <- last(RSI(store$cl[startIndex_macd:store$iter,i],n=params$lookback_macd[i])) 
        rsi1 <- last(RSI(store$cl[startIndex1_macd:store$iter,i],params$lookback1_macd[i]))
        rsi2 <- last(RSI(store$cl[startIndex2_macd:store$iter,i],params$lookback2_macd[i]))
        
        macdData <- MACD(store$cl[startIndexm_macd:store$iter,i],nSlow=params$nSlow,nFast=params$nFast[i],nSig=params$nSig)
        #print(macdData)
        macd <- macdData[,1]
        signal <- macdData[,2]
        #short
        ...
        if (rsi > (50 + params$threshold)&&rsi1<rsi2)
        {
          ...
          if(store$cl[store$iter,i]<store$cl[store$iter-1,i]&&last(macd) < last(signal) && last(macd,n=2[-2]) > last(signal,n=2)[-2])
            pos[params$series[i]] <- - params$posSizes[params$series[i]]
        }
    ...
    for (i in 1:length(params$series)) {
      if(i==3|i==6)  {
        ...
        if (R1 < -50 - params$threshold1_wr[i]){
          ...
          if (last(MAclS1) > last(MAclL1) && last(MAclS1,n=2)[-2] < last(MAclL1,n=2)[-2] 
              || last(macd1) > last(signal1) && last(macd1,n=2)[-2] < last(signal1,n=2)[-2]){
            pos3[params$series[i]] <- positionSizes[params$series[i]] # long
            currentCashFlow3[store$iter,params$series[i]] <- -cl*positionSizes[params$series[i]]
            #cat("currentCashFlow12:",currentCashFlow12[store$iter,params$series[i]],"\n")
          }         
        }
        #When in overbought price level
        if (R1 > -50 + params$threshold1_wr[i]){
          ...
          if (last(MAclS1) < last(MAclL1) && last(MAclS1,n=2)[-2] > last(MAclL1,n=2)[-2] 
              || last(macd1) < last(signal1) && last(macd1,n=2[-2]) > last(signal1,n=2)[-2]){
            pos3[params$series[i]] <- -positionSizes[params$series[i]]  # short
            currentCashFlow3[store$iter,params$series[i]] <- cl*positionSizes[params$series[i]]
            #cat("currentCashFlow12:",currentCashFlow12[store$iter,params$series[i]],"\n")
          }
        }
        ...
        if (R2 < -50 - params$threshold2_wr[i]){
          ...
          if (last(MAclS2) > last(MAclL2) && last(MAclS2,n=2)[-2] < last(MAclL2,n=2)[-2] 
              || last(macd2) > last(signal2) && last(macd2,n=2)[-2] < last(signal2,n=2)[-2]){
            pos4[params$series[i]] <- positionSizes[params$series[i]] # long
            currentCashFlow4[store$iter,params$series[i]] <- -cl*positionSizes[i]
          }         
        }
        #When in overbought price level
        if (R2 > -50 + params$threshold2_wr[i]){
          ...
          if (last(MAclS2) < last(MAclL2) && last(MAclS2,n=2)[-2] > last(MAclL2,n=2)[-2] 
              || last(macd2) < last(signal2) && last(macd2,n=2[-2]) > last(signal2,n=2)[-2]){
            pos4[params$series[i]] <- -positionSizes[params$series[i]]  # short
            currentCashFlow4[store$iter,params$series[i]] <- cl*positionSizes[i]
          }
        } 
        ...
        if(currentPos[i]==0){
          if (cl > bbands["up"]&rocCalc>0) {
            pos5[params$series[i]] <- params$posSizes[params$series[i]]
            liqDay<- 50   
            
            currentCashFlow5[store$iter,params$series[i]] <- -cl*params$posSizes[params$series[i]]
            # print(1)
          }
          else if (cl < bbands["dn"]&rocCalc<0) {
            pos5[params$series[i]] <- -params$posSizes[params$series[i]]
        ...
        if(PD3 >= PD4){
          if(PD3 >= PD5){
            ...
            else{ marketOrders[params$series[i]] <- marketOrders5[params$series[i]]
