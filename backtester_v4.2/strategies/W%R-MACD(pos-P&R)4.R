...
getOrders <- function(store, newRowList, currentPos, params) {
  ...
  #lookbackM <- params$nSlow + params$nSig + params$nHold
  lookbackM <- params$nSlow + params$nSig
  
  #cat("lookback:",lookback,"\n") #for debug
  
  if (store$iter > max(params$lookbackR,lookbackM)) {
    #cat("iter:",store$iter,"\n")
    
    
    startIndexM <- store$iter - lookbackM
    ...
    for (i in 1:length(params$series)) {
      ...
      #for MACD
      macdData <- MACD(store$cl[startIndexM:store$iter,i],
