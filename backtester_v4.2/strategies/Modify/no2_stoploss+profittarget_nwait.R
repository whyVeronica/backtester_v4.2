  ...
  if (store$iter > max(maxLookback1,maxLookback2)) {  
    for (i in 1:length(params$series)) {
      if(i!=4&i!=8&i!=3&i!=6)  {
        ...
        MAclS1 <- SMA(store$cl[(startIndexS1-params$nWait1[params$series[i]]):store$iter,i],n=params$lookbackS1[i])
        MAclL1 <- SMA(store$cl[(startIndexL1-params$nWait1[params$series[i]]):store$iter,i],n=params$lookbackL1[i])
        ...
        if (params$nWait1[params$series[i]] == 1){
          #When oversold 
        ...
        MAclS2 <- SMA(store$cl[(startIndexS2-params$nWait2[params$series[i]]):store$iter,i],n=params$lookbackS2[i])
        MAclL2 <- SMA(store$cl[(startIndexL2-params$nWait2[params$series[i]]):store$iter,i],n=params$lookbackL2[i])
        ...
        #Take a position after the trend has been held for one more day
        if (params$nWait2[params$series[i]] == 1){
          #When oversold 
    ...
    for (i in 1:length(params$series)) {
      if(i==3|i==6)  {
        ...
        if(marketOrders[params$series[i]] == -currentPos[params$series[i]] 
           & PnL[params$series[i]] > params$profitTarget[params$series[i]]*posSizes[params$series[i]]) {
          cat("currentPos:",currentPos[params$series[i]],"\n")
          cat("It worked on series",i,"\n")
        }
