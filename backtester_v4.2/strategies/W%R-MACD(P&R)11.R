...
getOrders <- function(store, newRowList, currentPos, params) {
  ...
  if (store$iter > max(params$lookbackR,lookback)) {
    ...
    for (i in 1:length(params$series)) {
      #MAOpen <- store$op[(store$iter - lookback):i,]
      #largestOpen <- max(MAOpen)
      #largestOpen <- max(store$op)
      #openRets <- as.matrix(diff(log(store$op[(store$iter - lookback):i,])))
      ...
      if (params$nWait == 0){
        ...
        if (R < -50 - params$threshold){
          ...
          if (MAclS > MAclL || last(macd) > last(signal) && last(macd,n=2)[-2] < last(signal,n=2)[-2])
            marketOrders[params$series[i]] <- 1 # long
        }
        #When in overbought price level
        if (R > -50 + params$threshold){
          #when there's a bearish crossover
          if (MAclS < MAclL || last(macd) < last(signal) && last(macd,n=2[-2]) > last(signal,n=2)[-2])
            marketOrders[params$series[i]] <- -1  # short
        }
      }
