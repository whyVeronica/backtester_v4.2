...
checkParams <- function(params) { # make sure params are correct
  ...
  if (!"nWait" %in% names(params))
    stop("Parameter nWait not defined for strategy MACD")
  ...
  if (params$nWait < 0 || params$nWait > 3)
    stop("Parameter nWait can only be 0~3 (Longer holding period has not been coded yet).")
}
