...
getOrders <- function(store, newRowList, currentPos, params) {
  ...
  if (store$iter > max(params$lookbackR,lookback)) {
    ...
    for (i in 1:length(params$series)) {
      ...
      highma <- last(SMA(store$hi[startIndexR:store$iter,i], n=params$lookbackR))
      lowma <- last(SMA(store$lo[startIndexR:store$iter,i], n=params$lookbackR))
      spread <- sapply(1:length(newRowList),function(i)
        #params$spreadPercentage * (newRowList[[i]]$High - newRowList[[i]]$Low))
        maLogRet[i] * (highest - lowest))
        #params$spreadPercentage * (highest - lowest))
        #params$spreadPercentage * (highma-lowma))
      
      #MAPriceBase <- last(EMA(store$cl[startIndexLimit:store$iter,i],n=params$lookbackLimit))
      limitOrders1[i]  <- positionSizes[i] # BUY LIMIT ORDERS
      limitPrices1[i]  <- sapply(1:length(newRowList),function(i) 
        cl - spread[i]/2)
      limitOrders2[i]  <- -positionSizes[i] # SELL LIMIT ORDERS
      limitPrices2[i]  <- sapply(1:length(newRowList),function(i) 
        cl + spread[i]/2)
    }
    #print("positionSizes:")
    #print(positionSizes)
