...
plotResults <- function(dataList, results, plotType='ggplot2', titleString=NULL) {
    ...
    allseries <- do.call(cbind,cumpnlList) #Your data is organized by rows, and you want to assemble it into a data frame.
    colnames(allseries) <- 1:10
    ...
    if (plotType=='ggplot2') {
        ...
        colnames(toplot) <- c(paste("Active on", results$k, "% of days;"), # colname for aggregate series hacked 
                            paste(sprintf("%02d",1:10),":"))
        colnames(toplot) <- paste(colnames(toplot),apply(toplot,MARGIN=2,pdratioString))
        ...
        p1 <- p1 + geom_hline(yintercept = 0, colour="red", linetype = "longdash") # zero line 
        ...
        grid.arrange(p1,p2,heights=c(0.75,2),main=textGrob(titleString,gp=gpar(fontsize=20,font=1)))
        
    } else if (plotType=='chartSeries') {

        chartSeries(pfolioPnL$CumPnL,theme='white',name=paste('PD Ratio =',pfolioPdratio))
                                                                               
        pdratioList <- lapply(results$pnlList,function(x) pdratio(x$DailyPnL))               
        ...
        for (i in 1:length(results$pnlList))                                               
            ...
            plot(addTA(results$pnlList[[i]]$CumPnL,legend=paste('Series',i,': PD Ratio = ',pdratioList[[i]]))) 
        title(main=titleString,cex=1.5)
    }
